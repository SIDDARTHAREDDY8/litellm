...
@pytest.mark.asyncio
@pytest.mark.parametrize("output_field", (None, "max_tokens", "max_completion_tokens", "max_output_tokens"))
async def test_unspecified_output_does_not_reserve_model_maximum(output_field: str | None) -> None:
    payload: Final = {
        "messages": [{"role": "user", "content": "Hello"}],
        **({output_field: None} if output_field is not None else {}),
    }
    original: Final = _serialize(payload)
    budget: Final = ModelBudget("small-input-large-output", input_limit=500, output_limit=10_000)
    executor: Final = _Executor()
    result: Final = await prepare_compaction(payload, budget, _state(), (_SUMMARY,), executor, _Counter())
    assert result is None
    assert executor.calls.empty()
    assert _serialize(payload) == original
