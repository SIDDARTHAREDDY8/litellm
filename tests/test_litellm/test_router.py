...
@pytest.mark.parametrize("input_limit,output_limit", ((4096, 256), (0, 256), (4096, 0)))
def test_compaction_model_budget_uses_configured_limits(input_limit: int, output_limit: int) -> None:
    from litellm.router_strategy.complexity_router.context_compaction import CompactionFailure, ModelBudget

    router: Final = Router(model_list=[])
    deployment: Final[DeploymentTypedDict] = {
        "model_name": "budget-target",
    ...
    if input_limit == 0 or output_limit == 0:
        assert isinstance(budget, CompactionFailure)
    else:
        assert isinstance(budget, ModelBudget)
        assert (budget.input_limit, budget.output_limit) == (input_limit, output_limit)
        assert budget.model == deployment["litellm_params"]["model"]


def test_compaction_request_payload_does_not_mutate_request_metadata() -> None:
    router: Final = Router(model_list=[], default_litellm_params={"temperature": 0.2})
    deployment: Final[DeploymentTypedDict] = {
        "model_name": "payload-target",
        "litellm_params": {"model": "openai/gpt-5.6-luna", "temperature": 0.5},
        "model_info": {},
    }
    request: Final = {"metadata": {"session_id": "preserved-session"}, "temperature": 0.7}
    original: Final = copy.deepcopy(request)
    payload: Final = router._compaction_request_payload(deployment, request, messages=())
    metadata: Final = TypeAdapter(Mapping[str, object]).validate_python(payload["metadata"])
    assert payload["temperature"] == request["temperature"]
    assert metadata["session_id"] == "preserved-session"
    assert payload["metadata"] is not request["metadata"]
    assert request == original


@pytest.mark.asyncio
async def test_aprepare_selected_deployment_ignores_unarmed_compaction_state() -> None:
    from litellm.router_strategy.complexity_router.context_compaction import COMPACTION_STATE_KEY, CompactionState

    router: Final = Router(model_list=[])
    deployment: Final[DeploymentTypedDict] = {
        "model_name": "unknown-target",
        "litellm_params": {"model": "custom/unknown-token-limits"},
        "model_info": {},
    }
    state: Final = CompactionState(timeout=0)
    result: Final = await router._aprepare_selected_deployment(
        deployment, "unknown-target", {COMPACTION_STATE_KEY: state}, messages=()
    )
...
class _CompactionSummaryAccessRecorder(CustomLogger):
    def __init__(self, records: SimpleQueue[Mapping[str, object]]) -> None:
        self.records = records

    async def async_pre_call_deployment_hook(self, kwargs: Mapping[str, object], call_type: object) -> None:
        metadata: Final = TypeAdapter(Mapping[str, object]).validate_python(kwargs.get("metadata") or {})
        if metadata.get("model_group") == "overflow-summary":
            self.records.put(MappingProxyType(dict(metadata)))


def _access_group_compaction_router(invalid_authorized: bool) -> Router:
    return Router(
        model_list=[
            {
                "model_name": "streaming-overflow",
            ...
            {
                "model_name": "streaming-primary",
                "litellm_params": {
                    "model": "openai/gpt-5.6-luna",
                    "api_key": "test-key",
                    "api_base": "https://streaming-primary.invalid",
                },
                "model_info": {"max_input_tokens": 4096, "max_output_tokens": 1024},
            },
            *(
                {
                    "model_name": "overflow-summary",
                    "litellm_params": {
                        "model": "openai/gpt-5.6-luna",
                        "api_key": "test-key",
                        "api_base": f"https://{identifier}.invalid",
                    },
                    "model_info": {
                        "id": identifier,
                ...
                for identifier, limit, access_group in (
                    ("summary-allowed", 64000, "summary-access"),
                    ("summary-invalid", 0, "summary-access" if invalid_authorized else "denied-access"),
                )
            ),
        ],
        num_retries=0,
...
@pytest.mark.parametrize("permission", ("restricted", "invalid-authorized", "direct", "wildcard"))
async def test_autorouter_overflow_summary_candidates_match_sdk_access(
    bucket: str,
    permission: str,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(litellm, "disable_aiohttp_transport", True)
    records: Final[SimpleQueue[Mapping[str, object]]] = SimpleQueue()
    monkeypatch.setattr(litellm, "callbacks", [_CompactionSummaryAccessRecorder(records)])
    router: Final = _access_group_compaction_router(permission == "invalid-authorized")
    _cool_down(router, "summary-invalid")
    grants: Final = ["overflow-summary"] if permission == "direct" else ["*"] if permission == "wildcard" else []
    auth: Final = UserAPIKeyAuth(
        models=grants,
        team_models=["summary-access"],
        budget_reservation={"reserved_cost": 1.0},
    )
    metadata: Final = {"user_api_key_auth": auth}
    params: Final = {
        **(
            {"metadata": {"application": "test"}, "litellm_metadata": metadata}
            if bucket == "both"
            else {bucket: metadata}
        ),
        "disable_fallbacks": True,
    }
    with respx.mock(assert_all_called=False) as transport:
        summary: Final = transport.route(host="summary-allowed.invalid").mock(side_effect=_overflow_provider_response)
        invalid: Final = transport.route(host="summary-invalid.invalid").mock(side_effect=_overflow_provider_response)
        target: Final = transport.route(host="streaming-primary.invalid").mock(side_effect=_overflow_provider_response)
        if permission == "restricted":
            await _request_effective_overflow(router, "chat", _STREAMING_OVERFLOW_HISTORY, params)
            assert summary.call_count == target.call_count == 1
            assert records.qsize() == 1
            child_auth: Final = records.get_nowait().get("user_api_key_auth")
            assert isinstance(child_auth, UserAPIKeyAuth)
            assert child_auth.models == auth.models
            assert child_auth.team_models == auth.team_models
            assert child_auth.budget_reservation is None
        else:
            with pytest.raises(
                litellm.ContextWindowExceededError, match="ordinary model group with known token limits"
            ):
                await _request_effective_overflow(router, "chat", _STREAMING_OVERFLOW_HISTORY, params)
            assert summary.call_count == target.call_count == 0
...
@pytest.mark.asyncio
@pytest.mark.parametrize("invalid_authorized", (False, True))
async def test_autorouter_overflow_summary_candidates_match_reauthenticated_proxy_child(
    invalid_authorized: bool,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from fastapi import FastAPI

    from litellm.proxy.common_utils.context_compaction import proxy_summary_executor_scope
    from litellm.proxy.litellm_pre_call_utils import add_litellm_data_to_request

    monkeypatch.setattr(litellm, "disable_aiohttp_transport", True)
    router: Final = _access_group_compaction_router(invalid_authorized)
    _cool_down(router, "summary-invalid")
    auth: Final = UserAPIKeyAuth(models=["summary-access"], api_key="test-hash")
    reconstructed: Final[SimpleQueue[UserAPIKeyAuth]] = SimpleQueue()
    app: Final = FastAPI()

    @app.post("/v1/chat/completions")
    async def summary_child(request: Request) -> Mapping[str, object]:
        assert request.headers.get("authorization") == "Bearer test-caller"
        body: Final = TypeAdapter(dict[str, object]).validate_python(await request.json())
        wire_metadata: Final = TypeAdapter(Mapping[str, object]).validate_python(body["metadata"])
        assert "user_api_key_auth" not in wire_metadata
        child_auth: Final = UserAPIKeyAuth(models=["summary-access"], api_key="test-hash")
        data: Final = await add_litellm_data_to_request(
            body, request, child_auth, proxy_config=MagicMock(), general_settings={}
        )
        child_metadata: Final = TypeAdapter(Mapping[str, object]).validate_python(data["metadata"])
        reconstructed.put(TypeAdapter(UserAPIKeyAuth).validate_python(child_metadata["user_api_key_auth"]))
        response: Final = await router.acompletion(**data)
        assert isinstance(response, litellm.ModelResponse)
        return TypeAdapter(Mapping[str, object]).validate_python(response.model_dump())
    ...
    request: Final = Request(
        {
            ...
            "root_path": "",
            "query_string": b"",
            "server": ("proxy.invalid", 443),
            "app": app,
            "headers": [(b"authorization", b"Bearer test-caller")],
        }
    )
    params: Final = {
        "litellm_metadata": {"user_api_key_auth": auth},
        "proxy_server_request": {"url": "https://proxy.invalid/v1/messages", "method": "POST", "body": {}},
        "disable_fallbacks": True,
    }
    with respx.mock(assert_all_called=False) as transport, proxy_summary_executor_scope(request, params, None):
        summary: Final = transport.route(host="summary-allowed.invalid").mock(side_effect=_overflow_provider_response)
        invalid: Final = transport.route(host="summary-invalid.invalid").mock(side_effect=_overflow_provider_response)
        target: Final = transport.route(host="streaming-primary.invalid").mock(side_effect=_overflow_provider_response)
        if invalid_authorized:
            ...
            await _request_effective_overflow(router, "chat", _STREAMING_OVERFLOW_HISTORY, params)
            assert summary.call_count == target.call_count == reconstructed.qsize() == 1
            assert reconstructed.get_nowait().models == auth.models
        assert invalid.call_count == 0


@pytest.mark.parametrize("metadata", (None, {"session_id": "shadow-session"}))
def test_silent_experiment_snapshot_excludes_explicit_messages(metadata: Mapping[str, object] | None) -> None:
    from litellm.router import _silent_experiment_kwargs_snapshot

    messages: Final = [{"role": "user", "content": "prepared history"}]
    kwargs: Final = {"messages": messages, "metadata": metadata, "stream": False}
    snapshot: Final = _silent_experiment_kwargs_snapshot(kwargs)
    assert dict(snapshot) == {"metadata": metadata, "stream": False}
    assert kwargs["messages"] is messages


@pytest.mark.asyncio
@pytest.mark.parametrize("overflow", (False, True))
async def test_autorouter_overflow_silent_experiment_uses_prepared_history(
    overflow: bool, monkeypatch: pytest.MonkeyPatch
) -> None:
    from litellm.router_strategy.complexity_router.context_compaction import use_summary_executor
    from litellm.types.llms.openai import AllMessageValues

    monkeypatch.setattr(litellm, "disable_aiohttp_transport", True)
    router: Final = _streaming_overflow_router("chat", 4096, deployment_params={"silent_model": "streaming-backup"})
    summary: Final = _OverflowSummary()
    silent_dispatched: Final = asyncio.Event()
    history: Final = TypeAdapter(list[AllMessageValues]).validate_python(
        [
            {"role": "user", "content": "historical record " * 5000 if overflow else "The code is MARIGOLD-482"},
            {"role": "assistant", "content": "Recorded"},
            {"role": "user", "content": "What is the launch code?"},
            {
                "role": "assistant",
                "tool_calls": [
                    {"id": "lookup-code", "type": "function", "function": {"name": "lookup", "arguments": "{}"}}
                ],
            },
            {"role": "tool", "tool_call_id": "lookup-code", "content": "MARIGOLD-482"},
        ]
    )
    original: Final = copy.deepcopy(history)

    def silent_response(request: httpx.Request) -> httpx.Response:
        silent_dispatched.set()
        return _overflow_provider_response(request)

    with respx.mock(assert_all_called=True) as transport, use_summary_executor(summary):
        target: Final = transport.route(host="streaming-primary.invalid").mock(side_effect=_overflow_provider_response)
        silent: Final = transport.route(host="streaming-backup.invalid").mock(side_effect=silent_response)
        response: Final = await router.acompletion(
            model="streaming-overflow", messages=history, max_tokens=128, disable_fallbacks=True
        )
        assert isinstance(response, litellm.ModelResponse)
        await asyncio.wait_for(silent_dispatched.wait(), timeout=5)
        assert target.call_count == silent.call_count == 1
        target_payload: Final = TypeAdapter(Mapping[str, object]).validate_json(target.calls[0].request.content)
        silent_payload: Final = TypeAdapter(Mapping[str, object]).validate_json(silent.calls[0].request.content)
        prepared: Final = TypeAdapter(list[dict[str, object]]).validate_python(target_payload["messages"])
        assert silent_payload["messages"] == prepared
        assert prepared[-3:] == original[-3:]
        assert len(summary.requests) == int(overflow)
        if overflow:
            assert "untrusted user data" in json.dumps(prepared)
            assert "historical record" not in json.dumps(prepared)
        else:
