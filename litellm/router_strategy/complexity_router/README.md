...
The input budget reserves an explicitly requested or configured output limit and a safety margin.
When no output limit is set, only the margin is reserved; the model's maximum output capability is
not treated as a requested allocation, and provider defaults still apply. Compaction allows at most
